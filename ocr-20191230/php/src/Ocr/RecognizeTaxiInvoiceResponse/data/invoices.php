...
use AlibabaCloud\SDK\Ocr\V20191230\Ocr\invoices\invoiceRoi;
        ...
        if (isset($map['Items'])) {
            if (!empty($map['Items'])) {
                ...
                foreach ($map['Items'] as $item) {
                    $model->items[$n++] = null !== $item ? RecognizeTaxiInvoiceResponse\data\invoices\items::fromMap($item) : $item;
                }
            }
        }
        if (isset($map['InvoiceRoi'])) {
            $model->invoiceRoi = invoices\invoiceRoi::fromMap($map['InvoiceRoi']);
        }
